...
resource "aws_iam_policy" "my-s3-read-policy" {
  name        = "mlopsStacksAWS1108v1-terraform-state-manage"
  description = "S3 bucket access policy for managing terraform state for mlopsStacksAWS1108v1"
...
resource "aws_iam_user" "s3_bucket_cicd_user" {
  name = "mlopsStacksAWS1108v1-terraform-cicd-user"
}
